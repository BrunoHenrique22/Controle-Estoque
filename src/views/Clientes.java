...
public class Clientes extends JFrame {
	...
	public Clientes() {
		...
		JLabel lblCpf = new JLabel("CPF ou CNPJ:");
		lblCpf.setForeground(Color.WHITE);
		lblCpf.setFont(new Font("Arial", Font.PLAIN, 12));
		lblCpf.setBounds(401, 11, 79, 14);
		contentPane.add(lblCpf);

		txtCpf = new JTextField();
		txtCpf.setColumns(10);
		txtCpf.setBounds(502, 8, 130, 20);
		...
		JLabel lblWhatzap = new JLabel("Whatsapp:");
		lblWhatzap.setForeground(Color.WHITE);
		lblWhatzap.setFont(new Font("Arial", Font.PLAIN, 12));
		lblWhatzap.setBounds(423, 171, 73, 14);
		...
		btnBuscar.setBounds(89, 0, 33, 49);
		...
		txtZap.setBounds(493, 168, 103, 20);
		...
		table.setModel(new DefaultTableModel(
				...
				new String[] { "ID", "Nome", "CPF ou CNPJ" }));
	...
	void addCliente() {
		...
		if (txtNome.getText().isEmpty()) {
			...
			JOptionPane.showMessageDialog(null, "Preencha O CPF ou CNPJ");
			txtCpf.requestFocus();
			...
			String create = "insert into clientes (nome,cpf,fone,whatsapp,cep,email,endereco,numero,complemento,bairro,cidade,uf,obs) values (?,?,?,?,?,?,?,?,?,?,?,?,?)";
			try {
				...
				pst.setString(4, txtZap.getText());
				...
				JOptionPane.showMessageDialog(null, "Esse Cliente com Esse CPF OU CNJP já existe!!");
				txtCpf.setText(null);
				txtCpf.requestFocus();
				limpar();
	...
	private void updateClientes() {
		...
		if (txtNome.getText().isEmpty()) {
			...
			// LOGICA PRINCIPAL
			String update = "update clientes set cpf  = ?,fone  = ?,whatsapp = ?,cep = ?,email = ?,endereco = ?,numero = ?,complemento = ?,bairro = ?,cidade = ?,uf = ?,obs = ?  where id = ?";
			try {
				...
				pst.setString(3, txtZap.getText());
